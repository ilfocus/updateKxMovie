...
//        dispatch_release(_dispatchQueue);
    ...
    NSLog(@"KxMovieViewController---dealloc");
               ...
               withError: (NSError *) error
{
    ...
    if (!error && decoder) { // 无错误且KxMovieDecoder不为空
        ...
        if (self.isViewLoaded) {   // 控制器的view加载后，再设置播放器的view
...
- (void) setupPresentView
{
    ...
    UIView *frameView = [self frameView];
    frameView.contentMode = UIViewContentModeScaleAspectFit;// 将图片等比例拉伸，可能不会填充满整个区域
...
- (void) tick
{
    ...
    CGFloat interval = 0;
    if (!_buffered) {
        interval = [self presentFrame];// 返回的是当前播放的时间点
    }
    ...
    if (self.playing) {
        ...
        if (!leftFrames ||
            ...
            // 如果缓冲的时间比设定的最小的缓冲时间还要小，则再去异步加载数据
            [self asyncDecodeFrames];
        }
        
        const NSTimeInterval correction = [self tickCorrection];// 时间修正
        const NSTimeInterval time = MAX(interval + correction, 0.01);// 播放时间外加修正时间
...
- (CGFloat) presentVideoFrame: (KxVideoFrame *) frame
{
    if (_glView) {
        ...
        [_glView render:frame];// render:实施，着色
...
//    NSLog(@"_moviePostion---%f",_moviePosition);
